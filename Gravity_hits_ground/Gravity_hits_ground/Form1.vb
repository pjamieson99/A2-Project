Public Class Form1
    ...
    Private Sub Display_Paint(sender As Object, e As PaintEventArgs) Handles Display.Paint
        ...
        '    Else
        '        Line(x, 1).RightSide = False
        '        Line(x, 0).RightSide = False
        '        Line(x, 1).LeftSide = False
        ...
        '        If Line(x, 1).LeftSide = True And Line(x, 1).RightSide = False Then
        ...
        '                    LineDiff = Line(x, 1).px1 + Math.Sqrt(Body.RaiseDistance ^ 2 - (Body.connections(x).Y - Line(x, 1).py1) ^ 2)
        '                    Line(GroundLegs - y, 1).px1 = Line(x, 1).px1 + Math.Sqrt(Body.RaiseDistance ^ 2 - (Body.connections(x).Y - Line(x, 1).py1) ^ 2)
        '                    Body.AngleChange = Math.Acos(((Line(x, 1).py1 - Body.connections(x).Y) ^ 2 - (Body.RaiseDistance ^ 2) - (LineDiff ^ 2)) / (-2 * Body.RaiseDistance * LineDiff))
        '                    Exit For
        '                End If
        '            Next

        '        ElseIf Line(x, 1).RightSide = True And Line(x, 1).LeftSide = False Then
        '            For y = 0 To GroundLegs
        '                If Line(y, 0).LeftSide = True And Line(y, 0).CheckFloor(floor) = True Then
        '                    Body.RaiseDistance = Line(x, 1).px1 - Line(GroundLegs - y, 0).px1
        '                    LineDiff = Line(y, 1).px1 + Math.Sqrt(Body.RaiseDistance ^ 2 - (Body.connections(x).Y - Line(x, 1).py1) ^ 2)
        '                    Line(y, 1).px1 = Line(x, 1).px1 - Math.Sqrt(Body.RaiseDistance ^ 2 - (Body.connections(x).Y - Line(x, 1).py1) ^ 2)
        '                    If (Body.RaiseDistance ^ 2 - (Body.connections(x).Y - Line(x, 1).py1) ^ 2) < 0 Then
        '                        Console.ReadLine()
        '                    End If
        '                    Body.AngleChange = Math.Acos(((Line(x, 1).py1 - Body.connections(x).Y) ^ 2 - (Body.RaiseDistance ^ 2) - (LineDiff ^ 2)) / (-2 * Body.RaiseDistance * LineDiff))
        '                    Exit For
        ...
        '        ElseIf Line(x, 1).RightSide = True And Line(x, 1).LeftSide = True Then
        '            For y = 0 To GroundLegs
        '                Body.ypos1 = Convert.ToInt16(Line(y, 1).py1)
        '                Body.ypos2 = convert.ToInt16(Line(y, 1).py1)
        '            Next
        '        End If
        ...
        For x = 0 To GroundLegs
            If Line(x, 0).CheckFloor(floor) = True And Line(x, 0).px1 < Body.CoM.X Then
                Line(x, 0).LeftSide = True
            ElseIf Line(x, 0).CheckFloor(floor) = True And Line(x, 0).px1 > Body.CoM.X Then
                Line(x, 0).RightSide = True
            ElseIf Line(x, 0).CheckFloor(floor) = True And Line(x, 0).px1 = Body.CoM.X Then
                Line(x, 0).RightSide = True
                Line(x, 0).LeftSide = True
            ...
            If Line(x, 0).LeftSide = False And Line(x, 0).RightSide = True Then
                Body.FallRight()
            ElseIf Line(x, 0).LeftSide = True And Line(x, 0).RightSide = False Then
